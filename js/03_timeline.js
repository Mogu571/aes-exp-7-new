// -------------------------- 进度条更新函数 --------------------------
function updateExperimentProgress(completed, total) {
    const fill = document.getElementById("experiment-progress-fill");
    ...
    text.textContent = `已完成 ${completed} / ${total} 张图片`;
...
function buildTimeline() {
    ...
    const startExperimentTransition = {
        ...
        on_load: () => {
            ...
            // 显示进度条并初始化
            const bar = document.getElementById("experiment-progress-bar-container");
            if (bar) {
                bar.style.display = "block";
                updateExperimentProgress(0, IMAGE_LIST.length);
            }
        }
    };
    ...
    for (let i = 0; i < IMAGE_LIST.length; i++) {
        ...
        const beautyRatingTrial = {
            ...
            on_finish: (data) => {
                ...
                saveBackupData();
                // 更新进度条
                updateExperimentProgress(i + 1, IMAGE_LIST.length);
            }
        };
    ...
    const endTrial = {
        ...
        stimulus: `
            <div style="text-align: center; padding: 50px; background-color: #ffffff; border-radius: 15px; margin: 100px auto; max-width: 600px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.1); border: 1px solid #e9ecef;">
                <h2 style="font-size: 28px; color: #28a745; margin-bottom: 30px;">✓ 实验7已完成！</h2>
        ...
        on_load: () => {
            ...
            setTimeout(() => {
                ...
                document.getElementById("js-download-btn").addEventListener("click", () => {
                    ...
                    const fileName = `${GLOBAL_DATA.subjectName}_实验7数据_${timestamp}.txt`;
